...
export default function Scanner() {
  ...
  const [activeModel, setActiveModel] = useState(() => localStorage.getItem('gmodel') || '')
  const [keyStatus, setKeyStatus] = useState({ msg: localStorage.getItem('gmodel') ? `✓ Verified — ${localStorage.getItem('gmodel')}` : '', cls: localStorage.getItem('gmodel') ? 'ok' : '' })
  ...
  async function verifyKey() {
      ...
      for (let i = 0; i < models.length; i++) {
        ...
        try {
          ...
          if (tr.ok) {
            ...
            localStorage.setItem('gkey', apiKey.trim())
            localStorage.setItem('gmodel', model)
    ...
    } catch (e) { setKeyStatus({ msg: `Error: ${e.message}`, cls: 'err' }) }
  }
  ...
  function loadFile(file) {
    ...
    reader.onload = e => { setImgB64(e.target.result.split(',')[1]); setResult(null); setLogStatus(''); setAnalyzeStatus({ msg:'',cls:'' }) }
  ...
  async function analyze() {
    ...
    try {
      ...
      if (s === -1) throw new Error('Could not parse response.')
      setResult(JSON.parse(text.substring(s, e2 + 1)))
      setAnalyzeStatus({ msg: '', cls: '' })
    } catch (e) { setAnalyzeStatus({ msg: `Error: ${e.message}`, cls: 'err' }) }
    setAnalyzing(false)
  }
  ...
  async function handleAddToLog() {
    ...
    try {
      if (isPremium) {
        // Cloud save for premium users
        await addEntry({ ...result, dateKey, timeLabel })
      } else {
        // localStorage fallback for free users
        const entries = JSON.parse(localStorage.getItem(`log_${dateKey}`) || '[]')
        entries.push({ id: Date.now(), time: timeLabel, ...result })
  ...
  function mbar(label, val, unit, color, max) {
    return (
      <div key={label}>
        ...
        <div className="barbg"><div className="barfill" style={{ width:`${Math.min((val/max)*100,100)}%`, background:color, boxShadow:`0 0 8px ${color}60` }} /></div>
      </div>
    )
  }
  ...
  return (
    <div className="page">
      <div style={{ textAlign:'center', marginBottom:28 }}>
        <div style={{ fontSize:12, letterSpacing:'0.3em', color:'#ff6b35', textTransform:'uppercase', fontFamily:'DM Mono, monospace', marginBottom:10 }}>AI Nutrition Scanner</div>
      ...
      {!activeModel && (
        <div className="card">
          ...
          <button onClick={verifyKey} style={{ width:'100%', padding:13, background:'rgba(255,255,255,0.06)', border:'1px solid #2a2a3a', borderRadius:12, color:'#aaa', cursor:'pointer', fontFamily:'DM Mono, monospace', fontSize:13 }}>🔍 Verify Key</button>
        </div>
      )}

      {keyStatus.msg && <div className={`ibox ${keyStatus.cls}`}>{keyStatus.msg}</div>}

      <div style={{ border:`2px ${imgB64?'solid':'dashed'} ${imgB64?'#222':'#2a2a3a'}`, borderRadius:20, overflow:'hidden', minHeight:imgB64?'auto':180, display:'flex', alignItems:'center', justifyContent:'center', cursor:imgB64?'default':'pointer', background:'rgba(255,255,255,0.02)', marginBottom:16, position:'relative' }}
      ...
      {imgB64 && <button className="btn-main" onClick={analyze} disabled={analyzing||!activeModel}>
      ...
      {result && (
        <div className="fadeUp" style={{ marginTop:20 }}>
          ...
          <button className="btn-green" onClick={handleAddToLog} style={{ background:logStatus==='added'?'rgba(74,222,128,0.2)':undefined }}>
            {logStatus==='added' ? '✓ Added to Log!' : logStatus==='error' ? '✕ Error — try again' : isPremium ? '✚ Add to Log (Cloud)' : '✚ Add to Today\'s Log'}
          </button>
